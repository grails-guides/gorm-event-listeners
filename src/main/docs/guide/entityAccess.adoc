...
Create a new Grails service, called `SerialNumberService`:

[source,bash]
----
~ grails create-service demo.SerialNumberService
----

Edit the service as shown below, adding a single `@Listener` method:

[source,groovy]
.grails-app/services/demo/SerialNumberService.groovy
----
include::{sourceDir}/grails-app/services/demo/SerialNumberService.groovy[]
----
