...
Write a unit to verify the `auditDataService` is invoked whenever a `Book` event of type `PostInsertEvent`, `PostUpdateEvent` or `PostDeleteEvent` is received. The next illustrates another big advantage of GORM Data Services. They create easy to test scenarios. Being interfaces they are easy to Mock.
...
Next, create an integration tests to verify that saving, updating or deleting a book leaves an audit trail. Since we are capturing GORM Events asynchrounsly, we test using 
We use http://spockframework.org/spock/javadoc/1.1/spock/util/concurrent/PollingConditions.html[Spock PollingConditions] which repeatedly evaluates one or more conditions until they are satisfied or a timeout has elapsed.
